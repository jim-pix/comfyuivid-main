...
# Use libtcmalloc for better memory management (si disponible)
...
if [ ! -d "/workspace/models" ]; then
    ...
    echo "Please attach your Network Volume"
    exit 1
fi
...
if [ -d "/workspace/custom_nodes" ]; then
    ...
    for node_dir in /workspace/custom_nodes/*/; do
        if [ -f "${node_dir}requirements.txt" ]; then
            ...
            pip install -r "${node_dir}requirements.txt" --no-cache-dir --quiet || echo "    ⚠️ Failed for $node_name"
        fi
    done
    echo "✅ Custom nodes setup complete"
else
    echo "⚠️ Warning: No custom_nodes found"
fi
...
if [ "$SERVE_API_LOCALLY" == "true" ]; then
    python -u /comfyui/main.py --disable-auto-launch --disable-metadata --listen --verbose "${COMFY_LOG_LEVEL}" &
    echo "worker-comfyui: Starting RunPod Handler"
    python -u /handler.py --rp_serve_api --rp_api_host=0.0.0.0
else
    python -u /comfyui/main.py --disable-auto-launch --disable-metadata --verbose "${COMFY_LOG_LEVEL}" &
    echo "worker-comfyui: Starting RunPod Handler"
    python -u /handler.py
fi